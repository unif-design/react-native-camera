...
- **快门后逐张合成**：用 `@shopify/react-native-skia` 在全分辨率离屏 surface 上画原图 + 逐行画水印文字，编码为 JPEG，再用 `@dr.pogodin/react-native-fs` 写回临时文件；合成期间取景画面中央显示「水印生成中…」遮罩。
- **串行处理**：一次只烧一张，峰值内存恒定，不受连拍张数影响（Skia 原生对象用后即释放，避免大图反复烧导致 OOM）。
- **失败留在会话内重试**：解码 / 离屏分配 / 读写出现任何异常时，不交付未加水印的 raw 或半成品；相机回到可拍状态，保留此前文件并提示“照片处理失败，请重试”。录像不经过照片 processor。
