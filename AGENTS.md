...
你维护的仓库是 `react-native-camera`。本区块定义共享门禁;仓库正文只保存本仓特有规则,且只能补充或收紧共享规则。真实冲突必须如实报告,不得静默选择任一规则。
...
- 每次仓库改动(包括仅修改 `AGENTS.md`、CI 或仓库文档)都必须核对 `website`、`llms.txt` 和 `../skills/skills/camera/` 中对应的 `camera` Skill。
