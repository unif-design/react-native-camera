...
yarn test src/__tests__/example --runInBand  # example 纯逻辑与 App 行为
yarn test src/__tests__/exampleConfig.test.ts --runInBand  # example/native contract
...
- **仅新架构** —— 依赖 Nitro / vision-camera 5.x,旧架构(Bridge)不支持。**iOS 15.1+** / Android API 24+。(公共 RN peer 下限 0.85 所在的 RN 0.80+ 已把 `min_ios_version_supported` 抬到 `15.1`;当前仓库实际用 RN 0.86.2 验证。vision-camera / nitro / nitro-image / video / fs / blur 等 RN-core podspec 都继承该下限,Skia 写死 14.0、reanimated/worklets 13.4 更低,取**最高**即 15.1。)
...
- `jest.setup.ts` 只留本仓自己的**原生 peer** 桩:vision-camera 的 hooks(`useCameraPermission`/`useCameraDevice`/`usePhotoOutput`/`useVideoOutput`…)、nitro、skia、fs、video、svg、RNRC(本仓 `src/components/Carousel` 直接消费),使纯逻辑/组件测试能在无原生环境下跑。个别用例需要**驱动**手势或监视 reanimated 导出时,在该测试文件内 spread 真实模块再叠一层(见 `Camera.focus.test.tsx` / `Camera.aspectTransition.test.tsx`),不要改全局接线。
- **design 不桩** —— 接线交给 design 自己的 jest 入口后,整包 mock 就没有存在理由了:测试跑**真实** design 组件,断言按真实 a11y 语义写(`getByRole` + name)。Icon 这类装饰件整棵 a11y 子树隐藏(`accessibilityElementsHidden` + `importantForAccessibility="no-hide-descendants"`),按 testID 查要带 `{ includeHiddenElements: true }`;渲染树里有 `useTheme`/`useColors` 时补 `ThemeProvider` wrapper(缺 Provider 真实现会 warn),相机侧统一用 `__helpers__/renderDark`(恒 `forceScheme="dark"`)。
- `package.json#jest.moduleNameMapper` 把 `react` / design / RNGH / safe-area / reanimated / worklets 钉到**仓根**那份拷贝 —— `example/node_modules` 各有第二份,而官方桩只注册在被加载的那一份:两份混用时 RNGH 会在 example 那份上抛 `RNGestureHandlerModule could not be found`。不要再用 `jest.mock('./example/node_modules/…')` 那种双路径写法绕。
